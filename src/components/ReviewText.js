			...
			<div className="review-text-container">
				<div className="review-text-panel">
					<div className="review-text-artist-title">
						<h1 className="review-text-artist">{artist && titleCase(artist)}</h1>
						<h1 className="review-text-title"><i>{title && titleCase(title)}</i></h1>
					</div>
					<div className="review-text-image">	
						<img src={review_img} alt="album cover" className="review-text-image-cover"></img>
						<h6>{toUpper(label)} · {pub_year}</h6>
					</div>
				...
				<div className="review-text-body">
					<div className="review-text-by">
						...
						<h5 className="review-text-author-type">{author_type && titleCase(author_type)}</h5>
						<div className="review-text-genre">
							<h6>
								<span style={{color: "red", marginRight: "1em", display: "inline-block"}}>/</span>
								{toUpper(genre)}
							</h6>
						</div>	
						<h6 className="review-text-pub-date">{toUpper(formatDate(pub_date))}</h6>
					</div>
